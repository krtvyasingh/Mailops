...
import { ScheduledSendManager, UndoSendManager, SnoozeReminderManager, evaluateRuleSet, renderTemplate, ShortcutsRegistry, buildJwzThreads, BatchProcessor, VacationResponder, OfflineSyncManager, } from '../modules/productivity';
...
productivityRouter.post('/batch/execute', async (c) => {
    try {
        ...
        const itemsMap = new Map((body.items || []).map((it) => [it.id, it]));
