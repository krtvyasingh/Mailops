 ...
 */
export function createMentionNotifications(text, context, teamMembers) {
 ...
 */
export function highlightMentionsInText(text) {
    if (!text)
        return '';
    return text.replace(/(?:^|\s|[\(\[\{,;:])(@[a-zA-Z0-9_\.\-]+)/g, (match, p1) => match.replace(p1, `<span class="mention font-semibold text-blue-600 bg-blue-50 px-1 py-0.5 rounded">${p1}</span>`));
}
/**
