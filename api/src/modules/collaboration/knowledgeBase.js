...
export function searchArticles(query) {
    ...
    return Array.from(articles.values()).filter(a => a.title.toLowerCase().includes(q) ||
        a.content.toLowerCase().includes(q) ||
        a.tags.some(t => t.toLowerCase().includes(q)));
}
export function linkArticleToThread(articleId, threadId) {
...
export function getRelatedArticles(threadId) {
    const links = threadLinks.get(threadId) || [];
