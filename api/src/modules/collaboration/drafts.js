 ...
 */
export function acquireDraftLock(draft, userId, leaseDurationMs = 60_000, now = new Date()) {
 ...
 */
export function releaseDraftLock(draft, userId, now = new Date()) {
 ...
 */
export function updateDraftWithVersion(draft, expectedVersion, updates, now = new Date()) {
 ...
 */
export function setDraftReviewStatus(draft, newStatus, actorUserId, now = new Date()) {
 ...
 */
export function threeWayMerge(baseText, userAText, userBText) {
    ...
    while (oIdx < baseLines.length || aIdx < aLines.length || bIdx < bLines.length) {
        ...
        if (aLine === bLine) {
            ...
            if (aLine !== null)
                outputLines.push(aLine);
            if (oIdx < baseLines.length)
                oIdx++;
...
export class DraftsManager {
    ...
    lock(id, userId, leaseDurationMs) {
    ...
    setReview(id, status, actorUserId) {
        const draft = this.drafts.get(id);
        if (!draft)
            throw new Error(`Draft ${id} not found`);
