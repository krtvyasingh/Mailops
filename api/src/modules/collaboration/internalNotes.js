 ...
 */
export function resolveInternalNote(note, resolvedByUserId) {
    return {
        ...note,
        isResolved: true,
        resolvedByUserId,
 ...
 */
export function stripInternalNotesFromOutboundEmail(content) {
    if (!content)
        return '';
 ...
 */
export function buildNoteHierarchy(notes) {
...
export class InternalNotesManager {
    ...
    listForEmail(emailId) {
        return Array.from(this.notes.values())
            .filter(n => n.emailId === emailId)
            .sort((a, b) => a.createdAt.getTime() - b.createdAt.getTime());
    }
    getTreeForEmail(emailId) {
