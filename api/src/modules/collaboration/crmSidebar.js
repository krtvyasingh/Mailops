 ...
 */
export function extractCompanyFromEmail(emailAddr) {
 ...
 */
export function calculateRelationshipHealthScore(totalInteractions, lastContactedAt, inboundCount, outboundCount, now = new Date()) {
 ...
 */
export function aggregateContactProfile(emailAddr, emailHistory, existingContact, now = new Date()) {
    ...
    const relevantEmails = emailHistory.filter(e => e.fromAddr.toLowerCase().includes(normalizedEmail) || e.toAddr.toLowerCase().includes(normalizedEmail));
    let inboundCount = 0;
    let outboundCount = 0;
    ...
    const healthScore = calculateRelationshipHealthScore(totalInteractions, lastSeenAt, inboundCount, outboundCount, now);
    const recentThreads = relevantEmails
        ...
        direction: (e.direction || (e.fromAddr.toLowerCase().includes(normalizedEmail) ? 'inbound' : 'outbound')),
...
export class CRMContactManager {
    ...
    getEnrichedProfile(email, emailHistory) {
