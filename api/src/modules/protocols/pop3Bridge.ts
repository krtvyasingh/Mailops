...
export interface POP3Session {
  ...
  messages: { id: string; size: number; deleted: boolean; rawMime: string }[];
}

export function handlePOP3Command(session: POP3Session, line: string): { response: string; terminate?: boolean } {
  ...
  switch (upper) {
    ...
    case 'STAT':
      if (session.state !== 'TRANSACTION') return { response: '-ERR Not authorized' };
    ...
    case 'LIST':
      ...
      if (args[0]) {
        ...
        if (msg && !msg.deleted) return { response: `+OK ${args[0]} ${msg.size}` };
