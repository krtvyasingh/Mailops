 ...
 */
export async function translateIMAPCommand(line, context) {
    ...
    try {
        switch (upperCommand) {
            ...
            case 'LOGIN':
                if (args.length < 2)
                    return [`${tag} BAD Missing arguments`];
            ...
            case 'UID':
                ...
                if (args.length === 0)
                    return [`${tag} BAD Missing sub-command`];
                const subCommand = args[0].toUpperCase();
                const subArgs = args.slice(1);
                if (subCommand === 'FETCH')
                    return await handleFETCH(tag, subArgs, context, true);
                if (subCommand === 'SEARCH')
                    return await handleSEARCH(tag, subArgs, context, true);
                if (subCommand === 'STORE')
                    return await handleSTORE(tag, subArgs, context, true);
...
// Command Handlers
// ---------------------------------------------------------
function handleLIST(tag, args, context) {
...
function handleSELECT(tag, args, context) {
    if (!context.isAuthenticated)
        return [`${tag} NO Not authenticated`];
    if (args.length < 1)
        return [`${tag} BAD Missing mailbox name`];
...
async function handleFETCH(tag, args, context, isUid = false) {
    if (!context.selectedBox)
...
async function handleSEARCH(tag, args, context, isUid = false) {
    if (!context.selectedBox)
        return [`${tag} NO No mailbox selected`];
...
async function handleSTORE(tag, args, context, isUid = false) {
    if (!context.selectedBox)
        return [`${tag} NO No mailbox selected`];
    if (args.length < 3)
        return [`${tag} BAD Missing arguments`];
