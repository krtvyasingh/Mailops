 ...
 */
export async function handleJMAPRequest(request, context) {
...
// Method Handlers
// ---------------------------------------------------------
async function handleMailboxGet(args, context) {
    ...
    return {
        ...
        state: currentSessionState,
        list,
        notFound: ids ? ids.filter((id) => !mailboxes.find(m => m.id === id)) : []
    };
}
async function handleEmailGet(args, context) {
    ...
    const filteredList = list.map(email => {
        if (!properties)
            return email;
        const filtered = { id: email.id };
        for (const prop of properties) {
            if (prop in email)
                filtered[prop] = email[prop];
        }
        return filtered;
    });
    return {
        ...
        state: currentSessionState,
        list: filteredList,
        notFound: ids ? ids.filter((id) => !allEmails.find(e => e.id === id)) : []
    };
}
async function handleEmailQuery(args, context) {
...
async function handleEmailSet(args, context) {
