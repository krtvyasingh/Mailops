...
export interface MailServerSettings {
  ...
  imapHost?: string;
  imapPort?: number;
  imapSocketType?: 'SSL' | 'STARTTLS' | 'plain';
  smtpHost?: string;
  smtpPort?: number;
  smtpSocketType?: 'SSL' | 'STARTTLS' | 'plain';
  pop3Host?: string;
  pop3Port?: number;
}
 ...
 */
export function generateThunderbirdAutoconfigXml(settings: MailServerSettings): string {
 ...
 * Spec: [MS-OXDISCO]
 */
export function generateOutlookAutodiscoverXml(email: string, settings: MailServerSettings): string {
  const domain = settings.domain;
 ...
 */
export function generateAppleMobileConfig(email: string, settings: MailServerSettings): string {
  const domain = settings.domain;
