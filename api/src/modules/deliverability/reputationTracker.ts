...
export interface DeliverabilityMetrics {
  ...
  dkimPassRate: number; // 0 to 100
  spfPassRate: number;  // 0 to 100
  calculatedScore: number; // 0 to 100
  healthStatus: 'EXCELLENT' | 'GOOD' | 'WARNING' | 'CRITICAL';
}

export function calculateSenderScore(metrics: Omit<DeliverabilityMetrics, 'calculatedScore' | 'healthStatus'>): DeliverabilityMetrics {
  ...
  if (complaintRate > 0.3) score -= 40;
  else if (complaintRate > 0.1) score -= 20;

  // Penalize for authentication failures
  if (dkimPassRate < 95) score -= 15;
  if (spfPassRate < 95) score -= 15;

  const calculatedScore = Math.max(0, Math.min(100, score));

  let healthStatus: DeliverabilityMetrics['healthStatus'] = 'EXCELLENT';
  if (calculatedScore < 50) healthStatus = 'CRITICAL';
  else if (calculatedScore < 75) healthStatus = 'WARNING';
  else if (calculatedScore < 90) healthStatus = 'GOOD';
