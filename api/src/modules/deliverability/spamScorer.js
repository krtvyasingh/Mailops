...
export function getSuggestions(breakdown) {
    const suggestions = [];
    if (breakdown.allCapsScore > 0)
        suggestions.push("Reduce the use of ALL CAPS in the subject.");
    if (breakdown.exclamationScore > 0)
        suggestions.push("Remove excessive exclamation marks.");
    if (breakdown.triggerWordsScore > 0)
        suggestions.push("Remove common spam trigger words (e.g., 'free', 'urgent').");
    if (breakdown.linksScore > 0)
        suggestions.push("Reduce the number of links in your email.");
    if (breakdown.unsubscribeScore > 0)
        suggestions.push("Include a List-Unsubscribe header.");
    if (breakdown.shortenerScore > 0)
        suggestions.push("Avoid using URL shorteners like bit.ly.");
    if (breakdown.imageTextScore > 0)
        suggestions.push("Increase the amount of text relative to images.");
    return suggestions;
}
export function isLikelyToPassSpamFilter(score) {
    return score < 50;
}
