...
export interface BounceClassification {
  recipient: string;
  bounceType: 'hard_bounce' | 'soft_bounce' | 'mailbox_full' | 'policy_block';
  smtpCode: string;
  reason: string;
  shouldSuppress: boolean;
}

export function classifySMTPBounce(recipient: string, rawDsnMessage: string): BounceClassification {
