 ...
 */
export async function configureAgent(userId, rules, preferences) {
    const defaultPrefs = {
        tone: 'professional',
 ...
 */
export async function processIncomingEmail(email, agent) {
    if (!agent.enabled) {
 ...
 */
export function triageEmail(email, rules) {
    const actionsToTake = [];
    const processedRuleTypes = new Set(); // Prevent conflicting actions
 ...
 */
export async function generateAutoReply(email, agent) {
    ...
    if (agent.preferences.tone === 'casual')
        signoff = 'Thanks,\n[Agent Signature]';
    if (agent.preferences.tone === 'friendly')
        signoff = 'Warmly,\n[Agent Signature]';
    return `${greeting}\n\n${body}\n\n${signoff}`;
}
// --- Internal Helpers ---
function evaluateStringMatch(target, operator, value) {
