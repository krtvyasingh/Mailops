...
export interface SentimentDataPoint {
  ...
  label: 'positive' | 'neutral' | 'negative' | 'escalation';
}

export interface ThreadSentimentTrajectory {
  threadId: string;
...
export function calculateSentiment(text: string): { score: number; label: SentimentDataPoint['label'] } {
  ...
  for (const w of words) {
    ...
    if (ESCALATION_TRIGGERS.has(w)) isEscalation = true;
  }

  const total = pos + neg || 1;
  const score = Math.max(-1, Math.min(1, (pos - neg) / total));

  if (isEscalation && score < 0) return { score: -1, label: 'escalation' };
  if (score > 0.2) return { score, label: 'positive' };
  if (score < -0.2) return { score, label: 'negative' };
  return { score: 0, label: 'neutral' };
}

export function computeSentimentTrajectory(threadId: string, messages: { id: string; sender: string; timestamp: number; text: string }[]): ThreadSentimentTrajectory {
  const points: SentimentDataPoint[] = messages.map(m => {
  ...
  let trend: ThreadSentimentTrajectory['trend'] = 'stable';
  if (points.length >= 2) {
    const firstScore = points[0].score;
    const lastScore = points[points.length - 1].score;
    if (lastScore - firstScore > 0.3) trend = 'improving';
    else if (firstScore - lastScore > 0.3) trend = 'deteriorating';
  }
