...
export class BM25SearchEngine {
    ...
    findFuzzyTerm(term) {
        ...
        for (const vocab of this.vocabulary) {
            if (Math.abs(vocab.length - term.length) > 2)
                continue;
    ...
    search(query) {
        if (!query || !query.trim() || this.docs.length === 0)
            return [];
        let terms = [];
        let exactPhrase;
        let fromFilter;
        let toFilter;
        let hasAttachmentFilter;
        let isUnreadFilter;
        let isStarredFilter;
        ...
        for (let i = 0; i < this.docs.length; i++) {
            const doc = this.docs[i];
            if (fromFilter && !doc.from.toLowerCase().includes(fromFilter))
                continue;
            if (toFilter && !doc.to.toLowerCase().includes(toFilter))
                continue;
            if (hasAttachmentFilter !== undefined && doc.hasAttachment !== hasAttachmentFilter)
                continue;
            if (isUnreadFilter !== undefined && doc.isUnread !== isUnreadFilter)
                continue;
            if (isStarredFilter !== undefined && doc.isStarred !== isStarredFilter)
                continue;
            if (exactPhrase) {
