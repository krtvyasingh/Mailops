 ...
 */
export function categorizeEmail(meta, body = '') {
    ...
    if (isBulk) {
        ...
        else if (/(sale|discount|deal|offer|promo|coupon|store|shop|black friday|cyber monday|exclusive deal)/i.test(subjectLower) ||
            /(sale|discount|off\b|promo code|voucher|special offer)/i.test(bodyLower)) {
            category = 'Promotions';
        }
        else if (fromLower.includes('forum') ||
    ...
    else if (/(invoice|receipt|statement|order #|tracking|security alert|password reset|verification code|billing|shipment|delivery)/i.test(subjectLower) ||
