 ...
 */
export function parseCsvString(csvText, delimiter = ',') {
    const rows = [];
    let currentRow = [];
    let currentCell = '';
    let inQuotes = false;
 ...
 */
export function extractTextFromAttachment(filename, contentType, rawContent) {
...
export class AttachmentInvertedIndex {
     ...
     */
    addDocument(attachmentId, emailId, filename, contentType, rawContent) {
    ...
    generateSnippet(text, matchedTerms, maxLen = 180) {
        if (!text)
            return '';
