...
export async function hashPasscode(passcode, salt) {
    ...
    const baseKey = await crypto.subtle.importKey('raw', encoder.encode(passcode), 'PBKDF2', false, ['deriveBits']);
 ...
 */
export async function verifyPasscode(passcode, saltBase64, expectedHash) {
 ...
 */
export async function createConfidentialMessage(params) {
    const id = nanoid();
    ...
    const encryptedBuffer = await crypto.subtle.encrypt({ name: 'AES-GCM', iv: iv }, aesKey, encodedContent);
 ...
 */
export async function accessConfidentialMessage(record, providedPasscode, now = new Date()) {
    if (!record) {
...
export async function createExpiringMessage(payload, options) {
...
export async function consumeExpiringMessage(record, pinOrPasscode) {
