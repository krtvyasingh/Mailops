 ...
 */
export function identifyMagicBytes(bytes) {
    if (bytes.length < 4) {
 ...
 */
export function scanZipArchiveEntries(bytes) {
 ...
 */
export function scanPdfForExploits(bytes) {
...
export function scanAttachment(fileOrBytes, bytesOrFile, declaredMime = 'application/octet-stream') {
    let filename;
    let contentBytes;
    if (typeof fileOrBytes === 'string') {
        filename = fileOrBytes;
        contentBytes = bytesOrFile || new Uint8Array(0);
    }
    else {
