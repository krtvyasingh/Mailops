 ...
 */
export function evaluateSpf(spfRecord, clientIp, senderDomain) {
 ...
 */
export function matchIpv4Cidr(ip, cidrOrIp) {
    try {
 ...
 */
export function parseDkimHeader(rawHeader) {
    ...
    const headerCanon = (cParts[0]?.toLowerCase() === 'relaxed' ? 'relaxed' : 'simple');
    const bodyCanon = ((cParts[1] || cParts[0])?.toLowerCase() === 'relaxed' ? 'relaxed' : 'simple');
 ...
 */
export function canonicalizeBodyRelaxed(body) {
    if (!body)
        return '';
 ...
 */
export function canonicalizeBodySimple(body) {
    if (!body)
        return '';
    const normalized = body.replace(/\r\n/g, '\n').replace(/\r/g, '\n');
    const lines = normalized.split('\n');
 ...
 */
export async function computeBodyHashSha256(canonicalBody) {
 ...
 */
export function canonicalizeHeaderRelaxed(name, value) {
 ...
 */
export async function verifyDkimSignature(dkimHeader, headersMap, publicKeyPemOrBase64, bodyText) {
    try {
        // 1. Verify Body Hash
        ...
        let cryptoKey;
        try {
            cryptoKey = await crypto.subtle.importKey('spki', binaryDer, { name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-256' }, false, ['verify']);
        }
        catch {
        ...
        const valid = await crypto.subtle.verify('RSASSA-PKCS1-v1_5', cryptoKey, sigBytes, dataToVerify);
        return {
 ...
 */
export function evaluateDmarc(fromDomain, spfResult, dkimResult, dmarcRecord) {
    ...
    const policy = (tags.get('p')?.toLowerCase() || 'none');
 ...
 */
export function isOrganizationalDomainMatch(d1, d2) {
    if (d1 === d2)
        return true;
    const p1 = d1.split('.');
    const p2 = d2.split('.');
