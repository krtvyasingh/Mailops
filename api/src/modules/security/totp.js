 ...
 */
export function generateTotpSecret(numBytesOrEmail, issuer = 'Mailops') {
 ...
 */
export async function generateTotpCode(secretBase32, timestampMs = Date.now(), stepSeconds = 30, digits = 6) {
 ...
 */
export async function verifyTotpCode(secretBase32, userCode, timestampMs = Date.now(), window = 1, stepSeconds = 30, digits = 6) {
 ...
 */
export function generateOtpAuthUri(accountEmail, secretBase32, issuer = 'Mailops') {
...
export function generateQrSvg(text, size = 200) {
    ...
    const matrix = Array.from({ length: gridCount }, () => Array(gridCount).fill(false));
