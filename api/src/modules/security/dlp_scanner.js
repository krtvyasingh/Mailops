 ...
 */
export function validateLuhn(digitsOnly) {
 ...
 */
export function identifyCreditCardBrand(digits) {
    if (/^4/.test(digits))
        return 'Visa';
 ...
 */
export function maskCreditCard(raw) {
 ...
 */
export function isValidSsn(ssnRaw) {
...
export function scanDlp(text) {
    ...
    while ((ssnMatch = ssnRegex.exec(text)) !== null) {
        ...
        if (isValidSsn(rawMatch)) {
            ...
            const alreadyMatched = violations.some(v => v.startIndex <= ssnMatch.index && v.endIndex >= ssnMatch.index + rawMatch.length);
            if (!alreadyMatched) {
                violations.push({
                    category: 'ssn',
                    severity: 'critical',
    ...
    for (const pattern of SECRET_PATTERNS) {
        ...
        while ((match = pattern.regex.exec(text)) !== null) {
            ...
            const overlapping = violations.some(v => (startIndex >= v.startIndex && startIndex < v.endIndex) ||
