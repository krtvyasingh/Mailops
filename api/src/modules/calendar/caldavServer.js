...
export function createEvent(userId, title, startTime, endTime, attendees = []) {
    const id = generateId();
    const now = new Date();
    const event = {
        id,
...
export function listEvents(userId, rangeStart, rangeEnd) {
    const userEvents = [];
    for (const event of eventsStore.values()) {
        if (event.userId === userId) {
            if (rangeStart && event.endTime < rangeStart)
                continue;
            if (rangeEnd && event.startTime > rangeEnd)
                continue;
 ...
 */
export function handleCalDAVPropfind(path, userId) {
 ...
 */
export function handleCalDAVReport(userId, events) {
