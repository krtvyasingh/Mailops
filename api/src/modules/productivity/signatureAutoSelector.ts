...
export interface SignatureRule {
  ...
  matchType: 'internal' | 'external' | 'domain_match' | 'default';
  targetDomain?: string;
}

export function selectSignature(recipientEmail: string, userDomain: string, rules: SignatureRule[]): string {
  ...
  if (domainRule) return domainRule.signatureHtml;

  // 2. Check internal vs external
  if (isInternal) {
    const internalRule = rules.find(r => r.matchType === 'internal');
    if (internalRule) return internalRule.signatureHtml;
  } else {
    const externalRule = rules.find(r => r.matchType === 'external');
    if (externalRule) return externalRule.signatureHtml;
  }
