 ...
 */
export function executeSieveRules(email, rules) {
    const result = {
        action: 'deliver',
        flags: [],
        stopped: false,
...
function evaluateCondition(email, condition) {
    ...
    function safeWildcardMatch(str, pattern) {
 ...
 */
export function compileSieveScript(script) {
    const rules = [];
